...
function specialEventHandler ( text,text2,text3 )
	...
	if result and #result > 0 then
		if result[1].id == sid and teaDecode(result[1].pass ,"Trujeczka") == text3 then outputChatBox("* Podałeś(aś) takie same hasło !",client) return end
		--if (result[1].id == sid and result[1].pass ~= teaEncode(text,"Trujeczka")) then outputChatBox("*Podales bledne~aktualne haslo do zmiany!",client) end
		local query=exports["pystories-db"]:dbSet("UPDATE pystories_users SET pass=? WHERE id=?",teaEncode(text3,"jebaniidioci"),sid)
